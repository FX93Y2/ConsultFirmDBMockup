# Remove data from tables, and then reinsert data.
...
def truncate_snowflake_table_if_exists(cursor, table_name):
    """Truncate the Snowflake table if it exists."""
    cursor.execute(f"TRUNCATE TABLE {table_name}")
    logging.info(f"Truncated table {table_name} if it existed.")
...
def load_to_snowflake(data):
    ...
    for table, df in data.items():
        ...
        try:
            logging.info(f"Truncating table if it exists: {snowflake_table_name}")
            truncate_snowflake_table_if_exists(cursor, snowflake_table_name)
